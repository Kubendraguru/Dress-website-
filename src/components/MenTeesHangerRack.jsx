...
const MEN_TEES_HOTSPOTS = [
  { id: 0, key: 'men-tee-ivory-pleated-polo', name: 'Ivory Pleated Knit Polo', x: 11.7, y: 39.5, hex: '#f6f4ee' },
  { id: 1, key: 'men-tee-noir-zip-polo', name: 'Noir Quarter-Zip Knit Polo', x: 28.3, y: 39.5, hex: '#141416' },
  { id: 2, key: 'men-tee-stripe-knit', name: 'Block-Stripe Textured Crewneck', x: 46.5, y: 39.5, hex: '#f0ece1' },
  { id: 3, key: 'men-tee-luffy-anime', name: 'Luffy Anime Heavyweight Tee', x: 65.4, y: 39.5, hex: '#f5efe4' },
  { id: 4, key: 'men-tee-tokyo-racing', name: 'Tokyo Underground Racing Tee', x: 85.9, y: 39.5, hex: '#f5ecd5' }
];

export default function MenTeesHangerRack({ onAddToCart, onToggleWishlist, wishlist = [], onQuickView }) {
  const [activeIdx, setActiveIdx] = useState(0);
  const [selectedSize, setSelectedSize] = useState('M');
  const [cardSelectedSizes, setCardSelectedSizes] = useState({});
  const [isAutoStepping, setIsAutoStepping] = useState(false);

  const tees = MEN_TEES_HERO;
  const activeTee = tees[activeIdx] || tees[0];
  ...
  useEffect(() => {
    ...
    if (isAutoStepping) {
      interval = setInterval(() => {
        setActiveIdx((prev) => (prev + 1) % tees.length);
      }, 4000);
    }
    return () => {
      if (interval) clearInterval(interval);
    };
  }, [isAutoStepping, tees.length]);

  const handlePrev = () => {
    setActiveIdx((prev) => (prev - 1 + tees.length) % tees.length);
  };
  ...
  const handleCardSizeSelect = (teeId, size) => {
    ...
    <div className="max-w-[1400px] mx-auto px-4 sm:px-8 pt-4 pb-12 sm:pb-16 relative z-10 select-none">
      ...
      <div className="text-center mb-7 sm:mb-9 select-none">
        ...
        <p className="font-mono text-[11px] sm:text-xs uppercase tracking-[0.25em] text-neutral-500 font-medium mt-1.5 flex items-center justify-center gap-2">
          <span>&bull;&bull;</span>
          <span>SIGNATURE TEXTURED KNIT POLOS &amp; STREETWEAR TEES</span>
          <span>&bull;&bull;</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 lg:gap-16 xl:gap-20 items-start">
        ...
        <div className="lg:col-span-7 xl:col-span-7 relative bg-[#ede8de]/60 rounded-3xl p-4 sm:p-5 border border-neutral-300/60 shadow-sm overflow-hidden flex flex-col justify-between">
          ...
          <div className="relative w-full aspect-[3/2] sm:aspect-[4/3] rounded-2xl overflow-hidden shadow-md bg-neutral-900">
            <img 
              src={MEN_TEES_HERO_IMAGE_URL} 
              alt="Curated Men's T-Shirts & Polos on Wooden Clothes Rail" 
              className="w-full h-full object-cover object-center"
            />

            {/* Hotspot Markers */}
            {MEN_TEES_HOTSPOTS.map((spot) => {
          ...
          <div className="w-full mt-3.5 flex items-center justify-between text-xs text-neutral-600 px-1">
            <span className="font-mono text-[10px] uppercase tracking-wider text-neutral-500">
              Click any garment pin on the wooden rail to inspect styling
            </span>
      ...
      <div className="mt-12 sm:mt-16 pt-8 border-t border-neutral-300/60">
        ...
        <div className="flex items-center justify-between mb-6 px-1">
          <span className="text-xs font-mono uppercase tracking-[0.2em] text-neutral-600 font-bold flex items-center gap-2">
            ...
            <span>Men's T-Shirts &amp; Polos &bull; Atelier Catalog</span>
          </span>
          <span className="text-[11px] font-mono text-neutral-400">
            {tees.length} Curated Styles
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-8">
          {tees.map((t, idx) => {
            ...
            const currentCardSize = cardSelectedSizes[t.id] || 'M';
            const isWishlisted = wishlist.includes(t.id);
